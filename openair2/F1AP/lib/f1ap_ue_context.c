 ...
 */
 ...
 /* --- LTM / EarlySync helpers for Inter-gNB-DU LTM Handover (TS 38.401 8.2.1.5) --- */
 static f1ap_LTMInformation_Setup_t *cp_LTMInformation_Setup(const f1ap_LTMInformation_Setup_t *orig)
 {
   if (!orig)
     return NULL;
   f1ap_LTMInformation_Setup_t *cp = calloc_or_fail(1, sizeof(*cp));
   cp->LTMIndicator = orig->LTMIndicator;
   CP_OPT_BYTE_ARRAY(cp->ReferenceConfiguration, orig->ReferenceConfiguration);
   CP_OPT_BYTE_ARRAY(cp->cSIResourceConfigToAddModList, orig->cSIResourceConfigToAddModList);
   CP_OPT_BYTE_ARRAY(cp->cSIResourceConfigToReleaseList, orig->cSIResourceConfigToReleaseList);
   return cp;
 }
 
 static void free_LTMInformation_Setup(f1ap_LTMInformation_Setup_t *p)
 {
   if (!p)
     return;
   FREE_OPT_BYTE_ARRAY(p->ReferenceConfiguration);
   FREE_OPT_BYTE_ARRAY(p->cSIResourceConfigToAddModList);
   FREE_OPT_BYTE_ARRAY(p->cSIResourceConfigToReleaseList);
   free(p);
 }
 
 static bool eq_LTMInformation_Setup(const f1ap_LTMInformation_Setup_t *a, const f1ap_LTMInformation_Setup_t *b)
 {
   if (!a || !b)
     return a == b;
   if (a->LTMIndicator != b->LTMIndicator)
     return false;
   _F1_EQ_CHECK_OPTIONAL_IE(a, b, ReferenceConfiguration, eq_ba);
   _F1_EQ_CHECK_OPTIONAL_IE(a, b, cSIResourceConfigToAddModList, eq_ba);
   _F1_EQ_CHECK_OPTIONAL_IE(a, b, cSIResourceConfigToReleaseList, eq_ba);
   return true;
 }
 
 static f1ap_LTMConfigurationIDMappingList_t *cp_LTMConfigurationIDMappingList(const f1ap_LTMConfigurationIDMappingList_t *orig)
 {
   if (!orig || orig->list_count <= 0 || !orig->list_array)
     return NULL;
   f1ap_LTMConfigurationIDMappingList_t *cp = calloc_or_fail(1, sizeof(*cp));
   cp->list_count = orig->list_count;
   cp->list_array = calloc_or_fail((size_t)orig->list_count, sizeof(*cp->list_array));
   for (int i = 0; i < orig->list_count; ++i)
     cp->list_array[i] = orig->list_array[i];
   return cp;
 }
 
 static void free_LTMConfigurationIDMappingList(f1ap_LTMConfigurationIDMappingList_t *p)
 {
   if (!p)
     return;
   free(p->list_array);
 ...
 static bool eq_LTMConfigurationIDMappingList(const f1ap_LTMConfigurationIDMappingList_t *a, const f1ap_LTMConfigurationIDMappingList_t *b)
 {
   if (!a || !b)
     return a == b;
 ...
 static f1ap_EarlySyncInformation_Request_t *cp_EarlySyncInformation_Request(const f1ap_EarlySyncInformation_Request_t *orig)
 {
   if (!orig)
     return NULL;
   f1ap_EarlySyncInformation_Request_t *cp = calloc_or_fail(1, sizeof(*cp));
   CP_OPT_BYTE_ARRAY(cp->RequestforRACHConfiguration, orig->RequestforRACHConfiguration);
   cp->LTMgNB_DU_IDsList_count = orig->LTMgNB_DU_IDsList_count;
   if (orig->LTMgNB_DU_IDsList_count > 0 && orig->LTMgNB_DU_IDsList_array) {
     cp->LTMgNB_DU_IDsList_array = calloc_or_fail((size_t)orig->LTMgNB_DU_IDsList_count, sizeof(*cp->LTMgNB_DU_IDsList_array));
     for (int i = 0; i < orig->LTMgNB_DU_IDsList_count; ++i)
       cp->LTMgNB_DU_IDsList_array[i] = orig->LTMgNB_DU_IDsList_array[i];
   }
   return cp;
 }
 
 static void free_EarlySyncInformation_Request(f1ap_EarlySyncInformation_Request_t *p)
 {
   if (!p)
     return;
   FREE_OPT_BYTE_ARRAY(p->RequestforRACHConfiguration);
   free(p->LTMgNB_DU_IDsList_array);
   free(p);
 }
 
 static bool eq_EarlySyncInformation_Request(const f1ap_EarlySyncInformation_Request_t *a, const f1ap_EarlySyncInformation_Request_t *b)
 {
   if (!a || !b)
     return a == b;
   _F1_EQ_CHECK_OPTIONAL_IE(a, b, RequestforRACHConfiguration, eq_ba);
   if (a->LTMgNB_DU_IDsList_count != b->LTMgNB_DU_IDsList_count)
     return false;
   for (int i = 0; i < a->LTMgNB_DU_IDsList_count; ++i) {
     if (a->LTMgNB_DU_IDsList_array[i].lTMgNB_DU_ID != b->LTMgNB_DU_IDsList_array[i].lTMgNB_DU_ID)
       return false;
   }
   return true;
 }
 ...
 f1ap_ue_context_setup_req_t cp_ue_context_setup_req(const f1ap_ue_context_setup_req_t *orig)
 {
   ...
   /* Inter-gNB-DU LTM Handover optional IEs */
   if (orig->LTMInformation_Setup)
     cp.LTMInformation_Setup = cp_LTMInformation_Setup(orig->LTMInformation_Setup);
   if (orig->LTMConfigurationIDMappingList)
     cp.LTMConfigurationIDMappingList = cp_LTMConfigurationIDMappingList(orig->LTMConfigurationIDMappingList);
   if (orig->EarlySyncInformation_Request)
     cp.EarlySyncInformation_Request = cp_EarlySyncInformation_Request(orig->EarlySyncInformation_Request);
   return cp;
 }
 ...
 bool eq_ue_context_setup_req(const f1ap_ue_context_setup_req_t *a, const f1ap_ue_context_setup_req_t *b)
 {
   ...
   /* Inter-gNB-DU LTM Handover optional IEs */
   _F1_CHECK_EXP(eq_LTMInformation_Setup(a->LTMInformation_Setup, b->LTMInformation_Setup));
   _F1_CHECK_EXP(eq_LTMConfigurationIDMappingList(a->LTMConfigurationIDMappingList, b->LTMConfigurationIDMappingList));
   _F1_CHECK_EXP(eq_EarlySyncInformation_Request(a->EarlySyncInformation_Request, b->EarlySyncInformation_Request));
   return true;
 }
 ...
 void free_ue_context_setup_req(f1ap_ue_context_setup_req_t *req)
 {
   ...
   if (req->LTMInformation_Setup) {
     free_LTMInformation_Setup(req->LTMInformation_Setup);
     req->LTMInformation_Setup = NULL;
   }
   if (req->LTMConfigurationIDMappingList) {
     free_LTMConfigurationIDMappingList(req->LTMConfigurationIDMappingList);
     req->LTMConfigurationIDMappingList = NULL;
   }
   if (req->EarlySyncInformation_Request) {
     free_EarlySyncInformation_Request(req->EarlySyncInformation_Request);
     req->EarlySyncInformation_Request = NULL;
   }
 }
 
 /* --- UE Context Setup Response LTM optional IEs (TS 38.401 8.2.1.5, 9.2.2.2) --- */
 static f1ap_requestedTargetCellGlobalID_t *cp_requestedTargetCellGlobalID(const f1ap_requestedTargetCellGlobalID_t *orig)
 {
   if (!orig)
     return NULL;
   f1ap_requestedTargetCellGlobalID_t *cp = calloc_or_fail(1, sizeof(*cp));
   *cp = *orig;
   return cp;
 }
 
 static void free_requestedTargetCellGlobalID(f1ap_requestedTargetCellGlobalID_t *p)
 {
   free(p);
 }
 
 static bool eq_requestedTargetCellGlobalID(const f1ap_requestedTargetCellGlobalID_t *a, const f1ap_requestedTargetCellGlobalID_t *b)
 {
   if (!a || !b)
     return a == b;
   return eq_f1ap_plmn(&a->pLMN_Identity, &b->pLMN_Identity) && a->nRCellIdentity == b->nRCellIdentity;
 }
 
 static f1ap_EarlySyncInformation_t *cp_EarlySyncInformation(const f1ap_EarlySyncInformation_t *orig)
 {
   if (!orig)
     return NULL;
   f1ap_EarlySyncInformation_t *cp = calloc_or_fail(1, sizeof(*cp));
   cp->tCIStatesConfigurationsList_count = orig->tCIStatesConfigurationsList_count;
   cp->earlyULSyncConfig = NULL;
   cp->earlyULSyncConfigSUL = NULL;
   if (orig->tCIStatesConfigurationsList_count > 0 && orig->tCIStatesConfigurationsList_array) {
     cp->tCIStatesConfigurationsList_array = calloc_or_fail((size_t)orig->tCIStatesConfigurationsList_count, sizeof(*cp->tCIStatesConfigurationsList_array));
     for (int i = 0; i < orig->tCIStatesConfigurationsList_count; ++i) {
       cp->tCIStatesConfigurationsList_array[i].tCIStateID = orig->tCIStatesConfigurationsList_array[i].tCIStateID;
       cp->tCIStatesConfigurationsList_array[i].tCIState = copy_byte_array(orig->tCIStatesConfigurationsList_array[i].tCIState);
     }
   }
   CP_OPT_BYTE_ARRAY(cp->earlyULSyncConfig, orig->earlyULSyncConfig);
   CP_OPT_BYTE_ARRAY(cp->earlyULSyncConfigSUL, orig->earlyULSyncConfigSUL);
   return cp;
 }
 
 static void free_EarlySyncInformation(f1ap_EarlySyncInformation_t *p)
 {
   if (!p)
     return;
   if (p->tCIStatesConfigurationsList_array) {
     for (int i = 0; i < p->tCIStatesConfigurationsList_count; ++i)
       free_byte_array(p->tCIStatesConfigurationsList_array[i].tCIState);
     free(p->tCIStatesConfigurationsList_array);
   }
   FREE_OPT_BYTE_ARRAY(p->earlyULSyncConfig);
   FREE_OPT_BYTE_ARRAY(p->earlyULSyncConfigSUL);
   free(p);
 }
 
 static bool eq_EarlySyncInformation(const f1ap_EarlySyncInformation_t *a, const f1ap_EarlySyncInformation_t *b)
 {
   if (!a || !b)
     return a == b;
   if (a->tCIStatesConfigurationsList_count != b->tCIStatesConfigurationsList_count)
     return false;
   for (int i = 0; i < a->tCIStatesConfigurationsList_count; ++i) {
     if (a->tCIStatesConfigurationsList_array[i].tCIStateID != b->tCIStatesConfigurationsList_array[i].tCIStateID)
       return false;
     if (!eq_byte_array(&a->tCIStatesConfigurationsList_array[i].tCIState, &b->tCIStatesConfigurationsList_array[i].tCIState))
       return false;
   }
   _F1_EQ_CHECK_OPTIONAL_IE(a, b, earlyULSyncConfig, eq_ba);
   _F1_EQ_CHECK_OPTIONAL_IE(a, b, earlyULSyncConfigSUL, eq_ba);
   return true;
 }
 
 static f1ap_LTMConfiguration_t *cp_LTMConfiguration(const f1ap_LTMConfiguration_t *orig)
 {
   if (!orig)
     return NULL;
   f1ap_LTMConfiguration_t *cp = calloc_or_fail(1, sizeof(*cp));
   cp->sSBInformation = copy_byte_array(orig->sSBInformation);
   CP_OPT_BYTE_ARRAY(cp->referenceConfigurationInformation, orig->referenceConfigurationInformation);
   if (orig->completeCandidateConfigurationIndicator) {
     cp->completeCandidateConfigurationIndicator = calloc_or_fail(1, sizeof(*cp->completeCandidateConfigurationIndicator));
     *cp->completeCandidateConfigurationIndicator = *orig->completeCandidateConfigurationIndicator;
   } else
     cp->completeCandidateConfigurationIndicator = NULL;
   CP_OPT_BYTE_ARRAY(cp->lTMCFRAResourceConfig, orig->lTMCFRAResourceConfig);
   CP_OPT_BYTE_ARRAY(cp->lTMCFRAResourceConfigSUL, orig->lTMCFRAResourceConfigSUL);
   return cp;
 }
 
 static void free_LTMConfiguration(f1ap_LTMConfiguration_t *p)
 {
   if (!p)
     return;
   free_byte_array(p->sSBInformation);
   FREE_OPT_BYTE_ARRAY(p->referenceConfigurationInformation);
   free(p->completeCandidateConfigurationIndicator);
   FREE_OPT_BYTE_ARRAY(p->lTMCFRAResourceConfig);
   FREE_OPT_BYTE_ARRAY(p->lTMCFRAResourceConfigSUL);
   free(p);
 }
 
 static bool eq_LTMConfiguration(const f1ap_LTMConfiguration_t *a, const f1ap_LTMConfiguration_t *b)
 {
   if (!a || !b)
     return a == b;
   if (!eq_byte_array(&a->sSBInformation, &b->sSBInformation))
     return false;
   _F1_EQ_CHECK_OPTIONAL_IE(a, b, referenceConfigurationInformation, eq_ba);
   if ((a->completeCandidateConfigurationIndicator != NULL) != (b->completeCandidateConfigurationIndicator != NULL))
     return false;
   if (a->completeCandidateConfigurationIndicator && b->completeCandidateConfigurationIndicator
       && *a->completeCandidateConfigurationIndicator != *b->completeCandidateConfigurationIndicator)
     return false;
   _F1_EQ_CHECK_OPTIONAL_IE(a, b, lTMCFRAResourceConfig, eq_ba);
   _F1_EQ_CHECK_OPTIONAL_IE(a, b, lTMCFRAResourceConfigSUL, eq_ba);
   return true;
 }
 ...
 f1ap_ue_context_setup_resp_t cp_ue_context_setup_resp(const f1ap_ue_context_setup_resp_t *orig)
 {
   ...
   /* Inter-gNB-DU LTM Handover optional IEs */
   if (orig->requestedTargetCellGlobalID)
     cp.requestedTargetCellGlobalID = cp_requestedTargetCellGlobalID(orig->requestedTargetCellGlobalID);
   if (orig->EarlySyncInformation)
     cp.EarlySyncInformation = cp_EarlySyncInformation(orig->EarlySyncInformation);
   if (orig->LTMConfiguration)
     cp.LTMConfiguration = cp_LTMConfiguration(orig->LTMConfiguration);
   return cp;
 }
 ...
 bool eq_ue_context_setup_resp(const f1ap_ue_context_setup_resp_t *a, const f1ap_ue_context_setup_resp_t *b)
 {
   ...
   /* Inter-gNB-DU LTM Handover optional IEs */
   _F1_CHECK_EXP(eq_requestedTargetCellGlobalID(a->requestedTargetCellGlobalID, b->requestedTargetCellGlobalID));
   _F1_CHECK_EXP(eq_EarlySyncInformation(a->EarlySyncInformation, b->EarlySyncInformation));
   _F1_CHECK_EXP(eq_LTMConfiguration(a->LTMConfiguration, b->LTMConfiguration));
   return true;
 }
 ...
 void free_ue_context_setup_resp(f1ap_ue_context_setup_resp_t *resp)
 {
   ...
   if (resp->requestedTargetCellGlobalID) {
     free_requestedTargetCellGlobalID(resp->requestedTargetCellGlobalID);
     resp->requestedTargetCellGlobalID = NULL;
   }
   if (resp->EarlySyncInformation) {
     free_EarlySyncInformation(resp->EarlySyncInformation);
     resp->EarlySyncInformation = NULL;
   }
